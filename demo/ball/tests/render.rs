...
use glam::{Mat4, Vec3};
use runt_core::{Engine, InputEvent, InputTrace, Key, SimConfig};
...
fn render() -> Option<(Frame, Vec3, Vec3)> {
    ...
    Some((Frame { pixels, view_proj }, ball, pickup))
}
...
fn the_level_renders_a_green_world_with_gold_rings_in_it() {
    ...
    // camera facing the void — lands well under this.
    let clear = [
    ...
    let drawn = frame
        .pixels
        .chunks_exact(4)
        .filter(|px| (0..3).any(|c| (px[c] as i32 - clear[c] as i32).abs() > 2))
        .count();
    let coverage = drawn as f64 / (W * H) as f64;
    println!("game frame: {:.1}% covered", coverage * 100.0);
    assert!(
        coverage > 0.4,
